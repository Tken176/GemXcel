...
def _play_music_thread(music_path, vol):
    try:
        ...
        config.logger.warning("Lỗi phát nhạc nền: %s", e)
...
def _select_music(music_file: str, game_state) -> None:
    try:
        ...
        config.logger.warning("Lỗi chọn nhạc: %s", e)
...
def _open_integrated_gem_ai(game_state: 'GameState') -> None:
    try:
        ...
        if not refresh_token:
            config.logger.warning("Không tìm thấy refresh_token. Vui lòng đăng nhập lại.")
            return
        ...
        if not access_token:
            config.logger.warning("Không thể làm mới token. Vui lòng đăng nhập lại.")
            return
        ...
        config.logger.warning("Lỗi khi mở GEM AI: %s", e)
...
def _refresh_access_token(refresh_token: str) -> str:
    ...
    try:
        ...
        config.logger.warning("Lỗi khi refresh token: %s", e)
        return ""
...
def _save_new_session(access_token: str, refresh_token: str, full_response: dict) -> None:
    ...
    try:
        ...
        config.logger.warning("Lỗi khi lưu session mới: %s", e)
